                    ...
                    <div className="ml-4 pl-2 border-l border-sidebar-border">
                      {(item.items ?? []).map((subItem) => (
                        <SidebarMenuItem key={subItem.title}>
                          <Link
                            href={subItem.url}
                            className={`flex w-full items-center gap-2 font-medium rounded-md p-2 text-sm hover:bg-sidebar-accent ${isActive ? "bg-sidebar-selected" : ""}`}
                            onClick={() => handleItemClick(subItem.title)} // Set the active item on click
                          >
